...
def plot_Sqw_EKmap(S, p, newcalc, wr):
    ...
    en_file = os.path.join(cache_dir, 'ZnCVO_EKmap_En.pck')
    sqw_file = os.path.join(cache_dir, 'ZnCVO_EKmap_Sqw.pck')

    # Determine if we need to calculate
    # If newcalc is 1 or output files don't exist, we must calculate.
    ...
    if should_calculate:
        ...
        sym_hm_file = os.path.join(project_root_dir, 'cache', 'symbolic_matrices', 'ZnCVO_EKmap_HM.pck')
        cache_mode = wr
        ...
        # Initialize MagCalc
        calc = mc.MagCalc(spin_magnitude=S, hamiltonian_params=p, cache_file_base='ZnCVO_EKmap', 
