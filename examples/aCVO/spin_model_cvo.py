...
def rot_mat(atom_list, p):
    ...
    for ag in atom_list:
        omp = sp.Matrix(
            [
                [np.cos(ag * np.pi / 2), 0, np.sin(ag * np.pi / 2)],
                [0, 1, 0],
                [-np.sin(ag * np.pi / 2), 0, np.cos(np.pi / 2)],
            ]
        )
        rot_m.append(omp)
    return rot_m
...
def mpr(p):
    al = [
        ...
        1,
        -1,
    ]  # angle in a unit of pi/2, 1 along +a, -1 along -a
    mp = rot_mat(al, p)
    return mp
...
def Hamiltonian(Sxyz, p):
    ...
    for i in range(Nspin):
        ...
        HM = HM + gamma * mu * Sxyz[i][0] * H  # magnetic field is along the a-axis
    HM = HM.expand()
    return HM
